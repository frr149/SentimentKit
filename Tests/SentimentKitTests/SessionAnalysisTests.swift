...
struct SessionAnalysisTests {
    ...
    func sessionAnalysisTracksPatienceAndTopExpressionsFromRealAnalyzerOutput() {
        ...
        #expect(session.topExpressions[Expression(text: "mierda", type: .profanity, language: "es")] == 1)
        #expect(session.topExpressions[Expression(text: "joder", type: .profanity, language: "es")] == 1)
        #expect(session.topExpressions[Expression(text: "perfecto", type: .positive, language: "es")] == 1)
    }
}

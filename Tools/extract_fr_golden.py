#!/usr/bin/env python3
"""
Extract French golden messages from cardiffnlp/tweet_sentiment_multilingual.
"""
...
# Load FR dictionary expressions
...
fr_positive = load_expressions(DICT_DIR / "fr-positive.tsv")
fr_frustration = load_expressions(DICT_DIR / "fr-frustration.tsv")
fr_profanity = load_expressions(DICT_DIR / "fr-profanity.tsv")

# Remove ambiguous
AMBIGUOUS = set()
fr_positive_filtered = [e for e in fr_positive if e not in AMBIGUOUS]

print(f"Loaded FR expressions:")
print(f"  positive: {len(fr_positive_filtered)}")
print(f"  frustration: {len(fr_frustration)}")
print(f"  profanity: {len(fr_profanity)}")
...
def categorize(text):
    pos = find_matches(text, fr_positive_filtered)
    fru = find_matches(text, fr_frustration)
    pro = find_matches(text, fr_profanity)
    return pos, fru, pro


# Download dataset
print("\nDownloading French split from HuggingFace...")

train_path = hf_hub_download(
    repo_id="cardiffnlp/tweet_sentiment_multilingual",
    filename="data/french/train.jsonl",
    repo_type="dataset",
)
...
print(f"Loaded {len(tweets)} French training tweets")
